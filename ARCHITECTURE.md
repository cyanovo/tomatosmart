...
本项目是一个面向番茄温室管控的智能体平台，集成了物联网传感器采集、多智能体协作决策、知识库检索与实时设备控制能力。用户在 Vue 前端中配置智能体、知识库、工具、Skills、MCP 与 SubAgents；前端通过 `/api` 调用 FastAPI；后端服务层协调数据库、对象存储、向量库、图数据库、LangGraph 运行态和沙盒；长耗时智能体运行交给 worker 异步执行，并通过事件流回到前端。
