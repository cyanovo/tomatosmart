...
async def query_kb(kb_id: str, query_text: str, file_name: str | None = None, runtime: ToolRuntime = None) -> Any:
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("kb_id") == target_kb_id and isinstance(result.get("results"), list):
            output = SearchOutputSchema(**result)
        else:
            output = KnowledgeBase.build_search_output(target_kb_id, result)

        # 给每条结果加【来源 N】编号，方便 LLM 引用
        if isinstance(output, SearchOutputSchema):
            output_data = output.model_dump()
        elif isinstance(output, dict):
            output_data = dict(output)
        else:
            output_data = {"kb_id": target_kb_id, "results": []}

        for i, item in enumerate(output_data.get("results") or []):
            source_name = (item.get("metadata") or {}).get("source", "") or item.get("file_id", "")
            item["content"] = f"【来源 {i + 1}】{source_name}\n{item.get('content', '')}"
