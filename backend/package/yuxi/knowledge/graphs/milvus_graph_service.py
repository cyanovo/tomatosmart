...
class MilvusGraphService:
        ...
        *,
        ...
        try:
            with self.driver.session() as session:
                result = session.run(
                    self._build_query(label, keyword, limit, max_depth, exclude_chunk, entity_type),
                    keyword=keyword,
    ...
    async def get_entity_types(self, kb_id: str | None = None) -> list[dict]:
        """获取所有实体类型及数量，用于前端分类导航"""
        effective_kb_id = kb_id or self.kb_id
        if not effective_kb_id:
            return []
        label = safe_neo4j_label(effective_kb_id)
        ...
        RETURN n.label AS entity_type, count(n) AS cnt
        ORDER BY cnt DESC
        """
        try:
            records = neo4j_read(self.driver, cypher, kb_id=effective_kb_id)
    ...
    @staticmethod
    def _build_where(exclude_chunk: bool, keyword: str, entity_type: str = "") -> str:
        clauses = []
        if exclude_chunk:
            clauses.append("NOT n:Chunk")
        if entity_type:
            clauses.append("toLower(coalesce(n.label, '')) = toLower($entity_type)")
        elif keyword and keyword != "*":
            clauses.append(
                "(toLower(coalesce(n.name, '')) CONTAINS toLower($keyword)"
                " OR toLower(coalesce(n.label, '')) CONTAINS toLower($keyword)"
    ...
    def _build_query(self, label: str, keyword: str, limit: int, max_depth: int, exclude_chunk: bool = False, entity_type: str = "") -> str:
        where = self._build_where(exclude_chunk, keyword, entity_type)
