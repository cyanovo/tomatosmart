...
JWT_AUDIENCE = "greenhouse-api"
PUBLIC_DEFAULT_JWT_SECRET_KEY = "greenhouse_secure_key"
PASSWORD_HASHER = PasswordHasher()


def _is_production_env() -> bool:
    return os.environ.get("GREENHOUSE_ENV", "development").strip().lower() in {"prod", "production"}
...
def _get_jwt_issuer() -> str:
    instance_id = _get_or_create_dev_env("GREENHOUSE_INSTANCE_ID", lambda: f"instance-{secrets.token_hex(8)}")
    return f"greenhouse-know:{instance_id}"
