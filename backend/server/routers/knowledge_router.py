...
MINDMAP_TASK_TYPE = "mindmap_generate"
...
async def generate_mindmap(
    ...
    model_spec: str = Body(default="", description="模型规格，为空则使用默认模型"),
    current_user: User = Depends(get_admin_user),
):
    """使用 AI 分析知识库文件，生成思维导图结构（后台任务）。"""

    async def _run_mindmap(context: TaskContext) -> dict:
        await context.set_progress(10.0, "正在分析知识库文件...")
        result = await generate_database_mindmap(kb_id, file_ids, user_prompt, model_spec)
        await context.set_progress(100.0, "思维导图生成完成")
    ...
    try:
        ...
        task, created = await tasker.enqueue_unique_by_payload(
            name=f"思维导图生成 ({db_info.get('name', kb_id)})",
            task_type=MINDMAP_TASK_TYPE,
            payload={"kb_id": kb_id, "model_spec": model_spec},
            coroutine=_run_mindmap,
            payload_match={"kb_id": kb_id},
        )
