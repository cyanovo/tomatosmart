...
COPY --from=m.daocloud.io/ghcr.io/astral-sh/uv:0.7.2 /uv /uvx /bin/
COPY --from=m.daocloud.io/docker.io/library/node:24-slim /usr/local/bin /usr/local/bin
COPY --from=m.daocloud.io/docker.io/library/node:24-slim /usr/local/lib/node_modules /usr/local/lib/node_modules
COPY --from=m.daocloud.io/docker.io/library/node:24-slim /usr/local/include /usr/local/include
COPY --from=m.daocloud.io/docker.io/library/node:24-slim /usr/local/share /usr/local/share
