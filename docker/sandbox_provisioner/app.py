...
class LocalContainerProvisionerBackend:
    def __init__(self):
        ...
        self._container_prefix = os.getenv("DOCKER_SANDBOX_PREFIX", "greenhouse-sandbox")
    ...
    def list(self) -> list[SandboxRecord]:
        containers = self._client.containers.list(
            all=True, filters={"label": ["app=greenhouse-sandbox", "managed-by=greenhouse-sandbox-provisioner"]}
        )
...
class KubernetesProvisionerBackend:
    def __init__(self):
        ...
        self._lock = threading.Lock()
        self._namespace = os.getenv("K8S_NAMESPACE", "greenhouse")
        self._sandbox_image = os.getenv(
            "SANDBOX_IMAGE",
            "enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest",
        )
        self._skill_pvc = os.getenv("SKILLS_PVC", "greenhouse-skills")
        self._thread_pvc = os.getenv("THREAD_PVC", "greenhouse-thread")
        ...
        *,
        ...
        return self._client.V1Pod(
            metadata=self._client.V1ObjectMeta(
                name=pod_name,
                labels={"app": "greenhouse-sandbox", "sandbox-id": sandbox_id},
                annotations={
                    "thread-id": thread_id,
    ...
    def list(self) -> list[SandboxRecord]:
        ...
        try:
            pod_list = self._core_api.list_namespaced_pod(
                namespace=self._namespace,
                label_selector="app=greenhouse-sandbox",
            )
        except ApiException:
            return []
...
app = FastAPI(title="Greenhouse Sandbox Provisioner", lifespan=lifespan)
