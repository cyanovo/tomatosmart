...
perl -pi -e "s/\\\$\\{GREENHOUSE_VERSION:-[^}]+\\}/\\\${GREENHOUSE_VERSION:-${NEW_VERSION}}/g" \
...
# greenhouse 包版本
perl -0pi -e "s/(^name = \"greenhouse\"\nversion = \")[^\"]+/\${1}${NEW_VERSION}/m" \
    "${PROJECT_ROOT}/backend/uv.lock"
# greenhouse-workspace 版本
perl -0pi -e "s/(^name = \"greenhouse-workspace\"\nversion = \")[^\"]+/\${1}${NEW_VERSION}/m" \
    "${PROJECT_ROOT}/backend/uv.lock"
...
grep -E "image: greenhouse-api:" "${PROJECT_ROOT}/docker-compose.yml" | head -1 | sed 's/^/    /'

echo "  docker-compose.prod.yml (web):"
grep -E "image: greenhouse-web:" "${PROJECT_ROOT}/docker-compose.prod.yml" | head -1 | sed 's/^/    /'
