  ...
  <div class="graph-section" v-if="isGraphSupported">
    <div class="graph-container-compact">
      ...
      <div v-else class="graph-wrapper">
        ...
        <!-- 实体分类浮窗 -->
        <div v-if="graphReady && entityTypes.length" class="entity-type-panel">
          <div class="type-panel-header">实体分类</div>
          <div class="type-panel-list">
            <button
              v-for="et in entityTypes"
              :key="et.type"
              class="type-chip"
              :class="{ active: activeEntityType === et.type }"
              @click="selectEntityType(et.type)"
            >
              <span class="type-name">{{ et.type }}</span>
...
const graphReady = computed(
  () => isMilvus.value && Boolean(graphBuildStatus.value?.locked)
)
...
const graphDataEmptyTitle = computed(() => {
  if (activeEntityType.value) return `未找到"${activeEntityType.value}"类型实体`
  if (searchInput.value.trim()) return '未找到匹配实体'
  if (!graphLoaded.value) return '点击刷新加载图谱'
  return '暂无知识图谱'
})
const graphDataEmptyDescription = computed(() => {
  if (activeEntityType.value) return '该分类下暂无已索引的实体，换个分类试试。'
  if (searchInput.value.trim()) return '换个关键词或调整图谱设置后再搜索。'
  if (!graphLoaded.value) return '点击上方刷新按钮加载知识图谱数据。'
...
const loadGraph = async () => {
  ...
  try {
    const res = await unifiedApi.getSubgraph({
      ...
      node_label: activeEntityType.value ? '*' : (searchInput.value || '*'),
...
const loadEntityTypes = async () => {
  ...
  try {
    const res = await unifiedApi.getEntityTypes(kbId.value)
    if (res.success && res.data?.types) {
      entityTypes.value = res.data.types
    }
  } catch (e) {
...
const selectEntityType = (type) => {
  if (activeEntityType.value === type) {
...
const scheduleGraphLoad = (delay = 200) => {
  ...
  if (!isGraphSupported.value || !kbId.value) {
    if (loadRetries < MAX_LOAD_RETRIES) {
      loadRetries++
      pendingLoadTimer = setTimeout(() => scheduleGraphLoad(300), 300)
    }
    return
  }
