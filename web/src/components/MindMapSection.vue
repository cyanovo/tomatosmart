  ...
  <div class="mindmap-section">
    <div class="section-content">
      ...
      <div v-else-if="!mindmapData" class="empty-state">
        ...
        <div class="mindmap-model-select">
          <span class="model-select-label">选择模型</span>
          <ModelSelectorComponent
            :model_spec="modelSpec"
            placeholder="默认模型"
            @select-model="(spec) => (modelSpec = spec)"
          />
        </div>
        <button
          type="button"
        ...
        >
          <Sparkles :size="14" />
          <span>{{ generating ? '生成中...' : '生成思维导图' }}</span>
        </button>
      </div>
...
const generateMindmap = async () => {
  ...
  try {
    ...
    // 提交后台任务
    const resp = await mindmapApi.generateMindmap(
      props.kbId,
      [], // 使用所有文件
      '', // 无自定义提示
      modelSpec.value
    ...
    // 轮询任务状态
    const poll = async () => {
      const detail = await taskerApi.fetchTaskDetail(taskId)
      const task = detail.task
      if (!task) return setTimeout(poll, 1000)
