<template>
  ...
  <teleport to="body">
    <div
      v-if="citePopover.visible"
      class="cite-popover"
      :style="{ top: citePopover.top + 'px', left: citePopover.left + 'px' }"
      @click.stop
    >
      <div class="cite-popover-header">
        <span class="cite-popover-title">{{ citePopover.source }}</span>
        <button class="cite-popover-close" @click="citePopover.visible = false">&times;</button>
      </div>
      <div class="cite-popover-body">
        <p v-if="citePopover.quote">{{ citePopover.quote }}</p>
        <p v-else class="cite-popover-hint">引用自知识库文档，索引 #{{ citePopover.index }}</p>
      </div>
    </div>
  </teleport>
</template>
...
import { getCiteSource } from '@/stores/citeSources'
...
const handleCiteClick = (el, event) => {
  const rect = el.getBoundingClientRect()
  citePopover.source = el.getAttribute('source') || ''
  citePopover.index = el.textContent || ''

  // 从存储中查找原文
  const src = getCiteSource(citePopover.index)
  if (src && src.content) {
    citePopover.quote = src.content
  } else {
    citePopover.quote = el.getAttribute('quote') || ''
  }

  citePopover.top = rect.bottom + 6 + window.scrollY
  citePopover.left = Math.min(rect.left + window.scrollX, window.innerWidth - 340)
  citePopover.visible = true
}
...
const handleClick = (e) => {
  ...
  if (citeEl) {
    e.preventDefault()
    handleCiteClick(citeEl, e)
    return
  }
  // 点击空白关闭弹窗
  if (citePopover.visible) {
    citePopover.visible = false
  }
  // SVG 操作按钮
  handleSvgAction(e)
}
