...
import { setCiteSources } from '@/stores/citeSources'
...
export function useAgentStreamHandler({
  ...
  const handleStreamChunk = (chunk, threadId) => {
    ...
    switch (status) {
          ...
          if (toolMessage) {
            ...
            if (content != null) {
              ...
              if (typeof content === 'string') {
                try { const p = JSON.parse(content); results = p.results || []; } catch { /* not JSON */ }
              } else if (content && typeof content === 'object' && Array.isArray(content.results)) {
                results = content.results
              }
              if (results.length) {
                const sources = results.map((item, i) => ({
                  index: i + 1,
                  source: (item.metadata && item.metadata.source) || item.file_id || '',
                  content: (item.content || '').replace(/^【来源 \d+】.*?\n/, '')
                }))
                setCiteSources(sources)
              }
            }
          }
        }
      ...
      case 'cite_sources':
        if (chunk.cite_sources) {
          setCiteSources(chunk.cite_sources)
        }
        return false
